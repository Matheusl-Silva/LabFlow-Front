...
/** Valor do exame -> string para preencher o input (null/undefined vira ""). */
function toInputValue(value: ExamValue | undefined): string {
...
export default function EditarExameDinamicoPage() {
  ...
  return (
    <DynamicExamEditForm
      ...
      onSubmit={async (values) => {
        ...
        const data: ExamData = {};
        for (const key of Object.keys(exam.schema)) {
        ...
        try {
          ...
          router.push(`${routes.exames}/${idPaciente}/${exam.id}`);
        } catch (err) {
          toast.error(isApiError(err) ? err.message : "Falha ao atualizar exame.");
        }
      }}
    />
  );
...
function DynamicExamEditForm({
  ...
  const {
    ...
    defaultValues: {
      date: exam.date,
      responsibleId: exam.responsibleId != null ? String(exam.responsibleId) : "",
      preceptorId: exam.preceptorId != null ? String(exam.preceptorId) : "",
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Editar — ${templateName}`}
        description={`Exame #${exam.id} · Paciente ${nomePaciente(paciente)} (#${paciente.id})`}
        actions={
          <Button asChild variant="outline">
            <Link href={`${routes.exames}/${idPaciente}/${exam.id}`}>
              <ArrowLeft className="h-4 w-4" />
              Voltar
            </Link>
          </Button>
